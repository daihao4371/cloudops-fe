...
import {getK8sPodYamlOne} from "@/api/demo/system";
...
getK8sPodYamlOne(params).then((res) => {


  yamlStr.value = res
  code.value = res
  title.value = `查看pod的yaml 集群:${query.cluster} ns:${query.ns} pod:${query.podName}  `

}).catch((err) => {
  createMessage.error('获取podyaml失败', err);
  console.log('获取podyaml失败', err);
}).finally(() => {
});
...
function handleDownLoadYamlFile() {
  var yamlFileName = `${query.cluster}_${query.ns}_${query.podName}.yaml`
