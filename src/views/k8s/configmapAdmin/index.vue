<template>
  <div>
    ...
    <BasicTable @register="registerTable" v-if="clusterNsValue.length >0">
      <template #toolbar>
        <a-button type="danger" @click="handleBatchDelete">批量删除Configmap </a-button>
...
<!--        <a-button class="mr-2" type="primary" @click="batchTaintNodes"> 批量增删节点污点 </a-button>-->
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'name'">
          ...
          <router-link target="_blank"  :to="{path:'/k8s/configmapYaml',query:{name:record.name, ns:record.namespace,cluster:  clusterNsValue[0] }}">
...
<!--            <router-link target="_blank"  :to="{path:'/k8s/podTailLog',query:{podName:record.name, ns:record.namespace,container: item,cluster:  clusterNsValue[0] }}">-->
    ...
    <ViewDrawer @register="registerDrawer" @success="handleSuccess" />
    <EditDrawer @register="registerDrawerEdit" @success="handleSuccess" />
...
import {ref} from "vue";
import {
  k8sBatchDeleteConfigmap,
  getClusterNamespacesForCascader,
  getK8sConfigmapListByNs,
} from "@/api/demo/system";
import {useMessage} from "@/hooks/web/useMessage";
import { BasicTable, useTable, TableAction } from '/@/components/Table';
import {columnsConfigmap , searchFormSchemaPods} from "./data";
...
function handleDelete(record: Recordable){
  ...
  //
  k8sBatchDeleteConfigmap(reqData).then(() => {
    createMessage.success(`已成功删除k8s-Configmap`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('删除k8s-Configmap失败');
    })
    .finally(() => {
...
function handleBatchDelete() {
  ...
  if (rows.length ===0 ) {
    createMessage.error('请选择Configmap');
    return
  }
...
function handleBatchRestart() {
  ...
  k8sBatchRestartConfigmap(reqData).then(() => {
    createMessage.success(`已成功重启k8s-Configmap`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('重启k8s-Configmap失败');
    })
    .finally(() => {
      record.pendingStatus = false;
    });
