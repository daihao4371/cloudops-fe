<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate"> 新增yaml模板 </a-button>
      </template>
...
import { columnsYamlTemplate, searchFormSchema } from './data';
...
function handleDelete(record: Recordable) {
  ...
  deleteK8sYamlTemplate(record.id).then(() => {
    createMessage.success(`已成功删除k8s集群`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('删除k8s集群失败');
    })
    .finally(() => {
      record.pendingStatus = false;
    });
