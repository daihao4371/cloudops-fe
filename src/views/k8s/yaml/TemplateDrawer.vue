<template>
  ...
  >
    ...
    <a-col :span="24">
      ...
      <a-card title="k8syaml模板表单字段" :bordered=true :headStyle="{color:'#1271d4',fontSize: 20}"
              :bodyStyle="{padding:'0'}">
      ...
      <a-card title="yaml配置区" :bordered=true :headStyle="{color:'#f6ac0a',fontSize: 20}"
              :bodyStyle="{padding:'0'}">
        <Codemirror
          v-model:value="code"
          :options="cmOptions"
...
import {formSchemaTemplate} from './data';
import {BasicDrawer, useDrawerInner} from '/@/components/Drawer';

import {
  createMenu,
  createK8sYamlTemplate,
  promqlExprCheck,
  updateK8sYamlTemplate,
...
async function handleSubmit() {
  try {
    ...
    // TODO custom api
    var modFunc = createK8sYamlTemplate
    if (unref(isUpdate)) {
      modFunc = updateK8sYamlTemplate
    }
    // 发起请求
    // 判断一下menus的类型
