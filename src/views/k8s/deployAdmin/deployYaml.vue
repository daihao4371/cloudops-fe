...
import {getK8sDeployYamlOne} from "@/api/demo/system";
...
getK8sDeployYamlOne(params).then((res) => {


  yamlStr.value = res
  code.value = res
  title.value = `查看Deploy的yaml 集群:${query.cluster} ns:${query.ns} pod:${query.name}  `

}).catch((err) => {
  createMessage.error('获取Deploy yaml失败', err);
  console.log('获取Deploy yaml失败', err);
}).finally(() => {
});
...
function handleDownLoadYamlFile() {
  var yamlFileName = `${query.cluster}_${query.ns}_${query.name}.yaml`
