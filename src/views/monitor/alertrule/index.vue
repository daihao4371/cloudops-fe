<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate"> 新增告警规则 </a-button>
...
import {
  ...
  deleteRole,
  getProcessList,
  getMonitorAlertRuleList,
  deleteMonitorAlertRule,

  enableSwitchMonitorAlertRule,
  batchEnableSwitchMonitorAlertRule,
  batchDeleteMonitorAlertRule,
  setRoleStatus
...
function batchEnableSwitch() {
  ...
  const data ={}
  data["ids"] =keys
  batchEnableSwitchMonitorAlertRule(data).then(() => {
    createMessage.success(`已批量开关告警规则`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('批量开关告警规则失败');
    })
    .finally(() => {
    });
...
function batchDelete() {
  ...
  batchDeleteMonitorAlertRule(data).then(() => {
    createMessage.success(`已批量删除告警规则`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('批量删除告警规则失败');
    })
    .finally(() => {
    });
...
function changeEnable(record){
  ...
  enableSwitchMonitorAlertRule(record.id).then(() => {
    createMessage.success(`已成功开关告警规则`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('开关告警规则失败');
    })
    .finally(() => {
...
function handleDelete(record: Recordable) {
  ...
  deleteMonitorAlertRule(record.id).then(() => {
    createMessage.success(`已成功删除告警规则`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('删除告警规则失败');
    })
    .finally(() => {
      record.pendingStatus = false;
    });
