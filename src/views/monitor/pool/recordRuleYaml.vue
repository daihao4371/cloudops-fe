...
import {getMonitorPrometheusRecordYamlOne} from "@/api/demo/system";
...
getMonitorPrometheusRecordYamlOne(query.ip).then((res) => {


  yamlStr.value = res
  code.value = res
  title.value = `预聚合触发器${query.poolName} 实例ip: ${query.ip}`

}).catch((err) => {
  createMessage.error('获取预聚合触发器失败', err);
  console.log('获取预聚合触发器失败', err);
}).finally(() => {
});
