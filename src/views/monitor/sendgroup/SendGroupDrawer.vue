...
const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
  ...
  console.log("发送组模态框-isUpdate",isUpdate)
  if (unref(isUpdate)) {
    getTitle.value = '编辑发送组'
...
async function handleSubmit() {
  try {
    ...
    // TODO custom api
    var modFunc = createMonitorSendGroup
    if (unref(isUpdate)) {
      modFunc = updateMonitorSendGroup
    }
    // 发起请求
    // 判断一下menus的类型
    ...
    if (reqData["needUpgrade"] == 2){
      delete reqData["firstUserNames"]
    }

    console.log("发送组操作要发起请求了 reqData",reqData)
