<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate"> 新增发送组 </a-button>
      </template>
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'firstUpgradeUsers'">
          ...
          <li v-for="(item, index) in record.firstUpgradeUsers">
            <a-tag :color="tagsColorFunc(index)" >{{ item.username }}</a-tag>
          </li>
        ...
        <template v-if="column.key === 'poolName'">
          ...
          <router-link target="_blank"  :to="{path:'/monitor/alertmanager'}">
...
import ProcessDrawer from './SendGroupDrawer.vue';
...
function handleDelete(record: Recordable) {
  ...
  deleteMonitorSendGroup(record.id).then(() => {
    createMessage.success(`已成功删除发送组`);
    reload();
  })
    .catch(() => {
      createMessage.error('删除发送组失败');
    })
    .finally(() => {
