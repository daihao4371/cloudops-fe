<template>
  ...
  >
    ...
    <a-col :span="24">
      ...
      <a-card title="预聚合规则表单字段" :bordered=true :headStyle="{color:'#1271d4',fontSize: 20}"
              :bodyStyle="{padding:'0'}">
...
import {
  createMenu,
  createMonitorRecordRule,
  promqlExprCheck,
  updateMonitorRecordRule,
...
const [registerDrawer, {setDrawerProps, closeDrawer}] = useDrawerInner(async (data) => {
  ...
  console.log("预聚合规则模态框-isUpdate", isUpdate)
  if (unref(isUpdate)) {
    getTitle.value = '编辑预聚合规则'
...
async function handleSubmit() {
  try {
    if (code.value===""){
      createMessage.error(`请填写预聚合promql`);
      return
    }
    ...
    // TODO custom api
    var modFunc = createMonitorRecordRule
    if (unref(isUpdate)) {
      modFunc = updateMonitorRecordRule
    }
    // 发起请求
    // 判断一下menus的类型
    ...
    console.log("预聚合规则操作要发起请求了 reqData", reqData)
