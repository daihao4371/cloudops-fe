...
import {
  createMenu, createMonitorOndutyGroup,
  createMonitorAlertmanagerPool,
  updateMonitorAlertmanagerPool,
...
const getTitle = ref('新增alertmanager集群');
...
const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
  ...
  getTitle.value = '新增alertmanager集群'
  ...
  console.log("alertmanager集群模态框-isUpdate",isUpdate)
  if (unref(isUpdate)) {
    getTitle.value = '编辑alertmanager集群'
...
async function handleSubmit() {
  try {
    ...
    // TODO custom api
    var modFunc = createMonitorAlertmanagerPool
    if (unref(isUpdate)) {
      modFunc = updateMonitorAlertmanagerPool
    }
    // 发起请求
    // 判断一下menus的类型
    ...
    console.log("alertmanager集群操作要发起请求了 reqData",reqData)
