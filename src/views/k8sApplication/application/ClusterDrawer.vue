<template>
  ...
  >
    ...
    <a-col :span="24">
      ...
      <a-card title="k8s应用表单字段" :bordered=true :headStyle="{color:'#1271d4',fontSize: 20}"
              :bodyStyle="{padding:'0'}">
...
const [registerDrawer, {setDrawerProps, closeDrawer}] = useDrawerInner(async (data) => {
  ...
  console.log("k8s应用模态框-isUpdate", isUpdate)
  if (unref(isUpdate)) {
    getTitle.value = '编辑k8s应用'
...
async function handleSubmit() {
  try {
    ...
    console.log("k8s应用操作要发起请求了 reqData", reqData)
