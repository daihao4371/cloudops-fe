<template>
  ...
  >
    ...
    <a-col :span="24">
      ...
      <a-card title="k8s项目表单字段" :bordered=true :headStyle="{color:'#1271d4',fontSize: 20}"
              :bodyStyle="{padding:'0'}">
...
const [registerDrawer, {setDrawerProps, closeDrawer}] = useDrawerInner(async (data) => {
  ...
  console.log("k8s项目模态框-isUpdate", isUpdate)
  if (unref(isUpdate)) {
    getTitle.value = '编辑k8s项目'
...
async function handleSubmit() {
  try {
    ...
    // TODO custom api
    var modFunc = createK8sProject
    if (unref(isUpdate)) {
      modFunc = updateK8sProject
    }
    // 发起请求
    // 判断一下menus的类型
    ...
    console.log("k8s项目操作要发起请求了 reqData", reqData)
