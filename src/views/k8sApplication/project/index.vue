<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate"> 新增k8s项目 </a-button>
...
<!--          <a-switch checked-children="开启" un-checked-children="关闭" :checked="enableCheckFunc(record.enable)"   />-->
        ...
        <template v-if="column.key === 'name'">
          ...
          <router-link target="_blank"  :to="{path:'/k8sApplication/application',query:{projectId:record.id,cluster:  record.cluster }}">
        ...
        <template v-if="column.key === 'nameZh'">
          ...
          <router-link target="_blank"  :to="{path:'/k8sApplication/cronjob',query:{projectId:record.id,cluster:  record.cluster }}">

            <a-tag color="blue" >{{ record.nameZh }}</a-tag>
          </router-link>
        </template>
...
import ProcessDrawer from './ProjectDrawer.vue';
...
function batchEnableSwitch() {
  ...
  batchEnableSwitchMonitorRecordRule(data).then(() => {
    createMessage.success(`已批量开关k8s项目`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('批量开关k8s项目失败');
    })
    .finally(() => {
    });
...
function batchDelete() {
  ...
  deleteK8sProjectOne(data).then(() => {
    createMessage.success(`已批量删除k8s项目`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('批量删除k8s项目失败');
    })
    .finally(() => {
    });
...
function changeEnable(record){
  ...
  enableSwitchMonitorRecordRule(record.id).then(() => {
    createMessage.success(`已成功开关k8s项目`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('开关k8s项目失败');
    })
    .finally(() => {
...
function handleDelete(record: Recordable) {
  ...
  deleteK8sProjectOne(record.id).then(() => {
    createMessage.success(`已成功删除k8s项目`);
    location.reload()
  })
    .catch(() => {
      createMessage.error('删除k8s项目失败');
    })
    .finally(() => {
      record.pendingStatus = false;
    });
